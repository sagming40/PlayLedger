...
다시 로그인하면 공격자의 토큰만 죽은 상태로 남는다. (03 문서 2.2절)
...
| v1.2 | 2026-09-19 | 1.2절 개발 환경 다이어그램의 API 포트를 8000 → 8001로 정정. M0에서 포트를 변경할 때 05 문서만 고치고 이 다이어그램을 누락했음 (사유는 DEVLOG 2026-09-19 M0 완료 기록의 결정 기록) |
| v1.3 | 2026-09-20 | 4.1절에 재사용 감지(폐기된 refresh 토큰 재제출 시 해당 사용자 토큰 일괄 폐기) 단락 추가. 03 문서 2.2절이 `revoked_at`을 남기는 목적으로 이미 명시하고 있었으나 인증 흐름에는 그려져 있지 않았음 |
