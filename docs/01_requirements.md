...
>
> 작성일: 2026-08-13 / 개정일: 2026-09-21 / 상태: v1.8
...
- [x] 상태 관리 라이브러리 → **Pinia**
- [x] 통계 시각화 → **ECharts**
- [x] 장르 데이터 출처 (직접 입력 vs Steam 태그 활용) → **Steam 공식 장르 8개를 고정 목록으로 사용** (사용자 입력 불가. 목록과 선정 기준은 03 문서 2.5절)
...
| 2026-09-20 | v1.5 | 4.4절에 F-28(만료 토큰 정리) 추가 — `refresh_tokens`는 폐기 기록을 남기기 위해 행을 지우지 않으므로(03 문서 2.2절) 로그인할 때마다 누적된다. 정리 작업은 v1.0 범위 밖으로 둔다 |
| 2026-09-21 | v1.6 | 4.4절에 F-29(모든 기기에서 로그아웃) 추가 — 로그인은 기존 토큰을 폐기하지 않아 기기별 토큰이 공존하며, 로그아웃은 요청한 기기의 토큰만 폐기한다. 전체 폐기는 별도 기능이라 v1.0 범위 밖으로 둔다. 사유는 DEVLOG 2026-09-21 결정 기록 참고 |
| 2026-09-21 | v1.7 | 10장 장르 데이터 출처 결정 완료로 이관 (직접 입력 vs Steam 태그 → Steam 공식 장르 중 8개를 고정 목록으로). 자유 입력은 공유 테이블 `genres`를 오염시키고, Steam 태그는 분류가 너무 잘게 쪼개져 장르별 통계(F-11)가 의미를 잃음. 사유는 DEVLOG 2026-09-21 결정 기록 참고 |
| 2026-09-21 | v1.8 | 4.4절에 F-30(비슷한 게임 후보 제시) · F-31(게임 목록 검색 등록) 추가, R-04 대응에 정규화의 한계와 보완책 명시 — 제목 정규화는 같은 글자를 다르게 쓴 경우만 잡고, 뜻이 같은 다른 표기는 잡지 못함. 규칙이나 유사도로 자동 병합하면 다른 게임을 합치는 실수가 생겨 v1.0 범위에서는 하지 않음. 사유는 DEVLOG 2026-09-21 결정 기록 참고 |
