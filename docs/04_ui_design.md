...
토큰 발급 원리는 `ARCHITECTURE` 4.1절을 따른다. 이 절은 그동안 **화면이 어떻게 보이는지**만 정한다.
...
| 제목 (수정 모드) | 수정할 수 없다 (`ERD` 2.6절). 입력칸을 읽기 전용으로 두고, 안내 문구로 삭제 후 재등록을 알린다. 화면 처리는 M3에서 정한다 |
| 장르 (수정 모드) | 게임에 장르가 이미 있으면 변경되지 않는다 (`ERD` 2.5절 장르 연결 규칙). 화면 처리는 M3에서 정한다 |
...
항상 표시한다. 비공개 상태에서 동기화하면 실패 원인을 오류 메시지로 명확히 알려준다. (`REQUIREMENTS` R-01)
...
`REQUIREMENTS` 5.4절대로, 지표는 판단 보조이고 결정은 사람이 한다.
...
- 다크 모드 → `REQUIREMENTS` F-24
- 게임 커버 아트 → `REQUIREMENTS` F-22
- 검색 기능 (필터로 우선 대응)
...
| v1.1 | 2026-09-21 | S-04 · S-08 목업의 장르 예시를 고정 장르 목록(`ERD` 2.5절) 안의 값으로 수정 (`오픈월드` → 삭제, `로그라이크` → `액션`). 두 값은 Steam 장르가 아니라 사용자 태그라 장르 목록에서 제외됨 |
| v1.2 | 2026-09-22 | 문서 참조 표기를 번호에서 문서 이름 기준으로 통일(예: "01~05 문서" → `REQUIREMENTS` · `ARCHITECTURE` · `ERD` · `UI_DESIGN` · `MILESTONES`. 내용 변경 없음) |
| v1.3 | 2026-09-22 | 3.3절 규격 표에 "장르 (수정 모드)" 행 추가 — 게임에 장르가 이미 있으면 수정해도 변경되지 않음을 명시 (`ERD` 2.5절 장르 연결 규칙). 화면 처리 방식은 M3에서 결정 |
| v1.4 | 2026-09-23 | 3.3절 규격 표에 "제목 (수정 모드)" 행 추가 — 보유 기록의 게임은 바꿀 수 없으므로 수정 화면에서 제목을 읽기 전용으로 둔다 (`ERD` 2.6절). 화면 처리 방식은 M3에서 결정 |
