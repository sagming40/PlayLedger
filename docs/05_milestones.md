...
| M1 | 백엔드 기초 (인증 + CRUD + 테스트) | v1.0 | 1.5~2주 | 🔄 진행 중 |
...
- [x] 문서 01 ~ 04 새 스택 기준으로 개정 (`ERD.md` → `03_erd.md` 포함)
- [x] 문서 05 새 스택 기준으로 개정
- [x] `README` 개정
- [x] `DEVLOG`에 스택 전환 결정 기록
...
- [x] `ARCHITECTURE` v1.1에 루트 `.env` · `.gitattributes` 반영
...
| 기술 요소 | SQLAlchemy(async), Alembic, Pydantic, JWT, pytest, pytest-asyncio, httpx, GitHub Actions |

**작업 목록**

*모델 · 마이그레이션*
- [x] v1.0 범위 모델 정의 — `users`, `refresh_tokens`, `games`, `genres`, `game_genres`, `entries`
- [x] 제약조건 작명 규칙 정의 (`Base`의 `naming_convention`) *(계획에 없던 작업)*
- [x] 첫 마이그레이션 생성 후 `--sql` 옵션으로 실제 생성될 SQL 확인
- [x] CHECK · UNIQUE · 외래키 삭제 규칙이 DB에 반영됐는지 HeidiSQL로 확인
- [x] 초기 장르 데이터 입력 (시드)
- [x] `genres.steam_genre_id` 컬럼 추가 — M8 장르 자동 연결 기준 *(계획에 없던 작업)*
- [x] 마이그레이션 파일명 규칙을 날짜 · 시각 기준으로 변경 (`alembic.ini`의 `file_template`) *(계획에 없던 작업)*
- [x] 수동 등록 게임 제목에 부분 UNIQUE 추가 (`uq_games_title_norm_manual`) *(계획에 없던 작업)*

*인증*
- [x] 비밀번호 해싱 라이브러리 결정 (`REQUIREMENTS` 10장)
- [x] 회원가입 (email 소문자 정규화)
- [x] 이메일 중복 시 409 + `IntegrityError` 2차 처리 *(계획에 없던 작업)*
- [x] 로그인 — access 토큰 + refresh 쿠키 발급
- [x] refresh 쿠키 이름에 프로젝트 접두사 부여 (같은 `localhost`를 쓰는 다른 프로젝트와 덮어쓰기 방지)
- [x] 로그인 실패 응답 시간 통일 (없는 계정에도 더미 해시 검증) *(계획에 없던 작업)*
- [x] 토큰 재발급 (rotation)
- [x] 로그아웃 (refresh 토큰 폐기 + 쿠키 삭제)
- [x] `get_current_user` 의존성
- [x] `GET /api/auth/me` — `get_current_user` 동작 확인 및 M2 계정 정보 화면용 *(계획에 없던 작업)*

*게임 CRUD*
- [x] 제목 정규화 함수 (`title_norm`)
- [x] 이메일 정규화를 같은 곳으로 통합 (현재 `schemas/user.py`와 `routers/auth.py`에 규칙이 중복) *(계획에 없던 작업)*
- [x] 게임 중복 판별 (`services`)
- [x] 장르 목록 조회 API
- [x] 장르 연결 규칙 구현 — 게임에 장르가 0개일 때만 채움, 동시 요청 대비 행 잠금 (`ERD` 2.5절) *(계획에 없던 작업)*
- [x] 등록 시 장르 입력 (F-02)
- [x] 보유 기록 조회 서비스 — `get_entry` · `entry_exists` (`user_id` 키워드 전용 인자) *(계획에 없던 작업)*
- [x] 보유 기록 등록 · 목록 · 단건 · 수정 · 삭제 (`user_id` 필수 시그니처)

*테스트 · CI*
- [x] pytest 환경 구성 (테스트 전용 DB)
- [x] 스모크 테스트 4건 — 헬스체크 · 장르 시드 · 흔적 남기기 · 초기화 확인 *(계획에 없던 작업)*
- [ ] 사용자 격리 테스트
- [ ] 게임 중복 판별 테스트 (표기만 다른 제목 · 동시 등록 시 재조회 포함)
- [ ] 장르 연결 동시 실행 테스트 (행 잠금 — 후발 요청이 대기 후 무시하는지) *(계획에 없던 작업)*
- [ ] 토큰 만료 · rotation 테스트
...
- [x] 다른 사용자의 기록을 ID로 직접 요청하면 404
- [x] 폐기된 refresh 토큰으로 재발급을 요청하면 401
- [ ] GitHub Actions 초록불
- [x] Swagger에서 가입 → 로그인 → 게임 등록 흐름을 손으로 한 번 확인

> **남의 기록 요청에 403이 아니라 404를 주는 이유:** 403(권한 없음)은
> "그 기록이 존재하긴 한다"는 사실을 알려준다. 로그인 실패 메시지를 하나로
> 통일하는 것(`UI_DESIGN` 3.1절)과 같은 원리다.
>
...
**완료 기준**
- [ ] `UI_DESIGN` 3.2절의 상태별 카드 표시가 5가지 상태 모두 동작
...
**작업 목록**
- [ ] 테스트용 게임 데이터 20개 이상 입력 (시드 스크립트 — `find_or_create_game` 경유, `ARCHITECTURE` 2.2절)
- [ ] 집계 쿼리를 HeidiSQL에서 먼저 검증 (`ERD` 3장)
- [ ] 총계 (총 보유 / 미시작 수)
- [ ] 사장률
- [ ] 완주율 (분모에서 미시작 제외)
- [ ] 방치 게임 추출
...
- [ ] 다른 PC에서 클론해 `README`만 보고 `docker compose up`으로 실행됨
...
- [ ] 위시리스트 CRUD API
- [ ] 구매 전 경고 쿼리 HeidiSQL 검증 (`ERD` 3.4절)
- [ ] 구매 전 경고 API
- [ ] 구매 전환 API (트랜잭션)
- [ ] S-08 위시리스트 화면 + 경고 배지
- [ ] 구매 전환 다이얼로그
...
- [ ] 보유 게임 목록 조회
- [ ] 병합 규칙 구현 (`ARCHITECTURE` 5.1절)
- [ ] Steam 장르 자동 연결 (`steam_genre_id` 기준, ERD 2.5절)
- [ ] 수동 등록 게임과 Steam 게임 연결 규칙 (`ERD` 2.4절 — v1.0에서는 수동 게임끼리만 판별)
...
- [ ] 동기화 증가분 → 세션 생성 (기준점 규칙, `ARCHITECTURE` 5.1절)
- [ ] 수동 세션 추가 API
- [ ] 같은 날 합산 (upsert, `ERD` 3.6절)
- [ ] S-04 플레이 기록 영역
- [ ] S-05 활동 히트맵
- [ ] 세션 규칙 테스트
- [ ] `v2.0.0` 태그
...
- [ ] 수동 게임에 세션을 추가하면 기존 플레이타임에 더해짐 (`REQUIREMENTS` 5.5절)
...
**작업 목록**
- [ ] 계정 연결 정책 결정 (`REQUIREMENTS` 10장)
- [ ] Discord 개발자 포털에 앱 등록
- [ ] `oauth_accounts` 모델 + 마이그레이션
- [ ] OAuth 흐름 구현 (`ARCHITECTURE` 4.3절)
...
| 마감 없는 범위 확대 | 완성 시점 소실 | 새 아이디어는 구현하지 않고 `REQUIREMENTS` 4.4절에 추가만 한다 |

기술적 리스크(학습 곡선, 외부 API 제약 등)는 `REQUIREMENTS` 8장에서 관리한다.
...
| v1.0 | 2026-09-18 | 스택 전환(Vue 3 + FastAPI + PostgreSQL)에 따른 전면 개정. 마일스톤을 M0~M11 · 버전별(v1.0~v2.1)로 재구성, 일정 전제를 취업 후 자기계발 기준으로 변경, 재설계 준비 작업을 M0에 기록, 기술 리스크를 `REQUIREMENTS`로 이관하고 일정 리스크만 유지, 완료 정의에 CI 통과 · 버전 태그 추가. v0 진행 결과는 `v0-rn-django` 태그 참고 |
| v1.1 | 2026-09-19 | M0 완료 처리 — 작업 목록 · 완료 기준 체크박스 전부 체크, 2장 상태표 ✅로 변경. 계획에 없던 작업 4건 추가 기록(루트 `.env` 분리, `ARCHITECTURE` 문서 반영, 경로 별칭 설정, Vite 템플릿 잔재 제거). 완료 기준의 API 포트를 8000 → 8001로 정정(포트 충돌 회피, 사유는 `DEVLOG` 2026-09-19 결정 기록). 완료 정의 2번을 Git 전략 전환에 맞춰 "PR로 `main`에 병합"으로 변경. M1에 refresh 쿠키 이름 접두사 항목, M5에 폰트 내장 검토 항목 추가 |
| v1.2 | 2026-09-19 | M1 "모델 · 마이그레이션" 섹션 4/5 완료 처리 — 모델 6종 정의, 첫 마이그레이션 `--sql` 확인 및 적용, 제약 동작 검증. 계획에 없던 작업 1건 추가 기록(제약조건 작명 규칙 정의 — 검증 단계에서 어느 제약이 걸렸는지 판별하기 위해 필요했음). 초기 장르 시드는 게임 CRUD 착수 전까지로 미루고 미체크 유지. 2장 상태표 M1을 🔄 진행 중으로 변경 |
| v1.3 | 2026-09-20 | M1 "인증" 섹션 3/8 완료 처리 — 비밀번호 해싱 라이브러리 결정(argon2-cffi), 회원가입 API. 계획에 없던 작업 1건 추가 기록(이메일 중복 409 + `IntegrityError` 2차 처리 — 조회와 INSERT 사이 경합에 대비). 결정 사유는 `DEVLOG` 2026-09-20 결정 기록 참고 |
| v1.4 | 2026-09-20 | M1 "인증" 섹션 5/9 완료 처리 — 로그인 API, refresh 쿠키 접두사. 계획에 없던 작업 2건 추가 기록(로그인 실패 응답 시간 통일 — argon2가 느려 계정 유무로 응답 시간이 갈리는 문제, 실측 42ms/42ms로 확인 / 이메일 정규화 통합 — `schemas`와 `routers` 두 곳에 같은 규칙이 중복돼 한쪽만 바뀌면 가입은 되는데 로그인이 안 되는 사고가 가능. 게임 CRUD의 `title_norm` 작업과 함께 처리). 결정 사유는 `DEVLOG` 2026-09-20 결정 기록 참고 |
| v1.5 | 2026-09-21 | M1 "인증" 섹션 전 항목 완료 처리(10/10) — 토큰 재발급(rotation) · 로그아웃 · `get_current_user`. 완료 기준 "폐기된 refresh 토큰으로 재발급을 요청하면 401" 충족. 계획에 없던 작업 1건 추가 기록(`GET /api/auth/me` — `get_current_user`를 확인할 수단이 필요했고, M2 S-06 계정 정보 화면에서도 쓰임). 자동 테스트 항목(토큰 만료 · rotation 테스트)은 pytest 구성 후 체크. 사유는 `DEVLOG` 2026-09-21 기록 참고 |
| v1.6 | 2026-09-22 | M1 "모델 · 마이그레이션" 섹션 완료 처리(초기 장르 시드), 계획에 없던 작업 3건 추가 기록(`steam_genre_id` 컬럼 — M8 장르 자동 연결 기준 / 마이그레이션 파일명 규칙 — revision ID만으로는 순서 · 내용을 알 수 없음 / 수동 게임 제목 부분 UNIQUE — 조회와 등록 사이의 동시 요청 대비). M1 "게임 CRUD" 섹션 3/6 완료 처리(제목 정규화 · 이메일 정규화 통합 · 게임 중복 판별 서비스). 게임 중복 판별 테스트 범위 명시. M8에 Steam 장르 자동 연결 · 수동/Steam 게임 연결 항목 추가. 사유는 `DEVLOG` 2026-09-21 · 09-22 기록 참고 |
| v1.7 | 2026-09-22 | 문서 참조 표기를 번호에서 문서 이름 기준으로 통일(예: "01~05 문서" → `REQUIREMENTS` · `ARCHITECTURE` · `ERD` · `UI_DESIGN` · `MILESTONES`. 내용 변경 없음) |
| v1.8 | 2026-09-23 | M1 "게임 CRUD" 섹션 — 장르 목록 API · 등록 시 장르 입력(F-02) 완료 처리, 계획에 없던 작업 2건 추가 기록(장르 연결 규칙 구현 — 공유 데이터인 `games`의 장르를 한 사용자가 바꾸지 못하게 0개일 때만 채움 / 보유 기록 조회 서비스 — `user_id`와 `entry_id`가 모두 `int`라 위치 인자로는 순서가 바뀌어도 파이썬이 못 잡음). 보유 기록 CRUD는 등록만 완료돼 미체크 유지. "테스트 · CI"에 장르 연결 동시 실행 테스트 추가. 완료 기준 "Swagger에서 가입 → 로그인 → 게임 등록 흐름 확인" 충족. 사유는 `DEVLOG` 2026-09-22 · 09-23 기록 참고 |
| v1.9 | 2026-09-23 | M1 "게임 CRUD" 섹션 완료 처리 — 보유 기록 목록 · 단건 · 수정 · 삭제 구현으로 5개 항목 전부 충족. 완료 기준 "다른 사용자의 기록을 ID로 직접 요청하면 404" 충족(B 계정으로 GET · PATCH 모두 404, 없는 id의 404와 응답 동일). 보유 기록의 제목(`game_id`)은 수정 불가로 확정 — 사유는 `ERD` 2.6절과 `DEVLOG` 2026-09-23 결정 기록 참고 |
| v1.10 | 2026-09-25 | M1 "테스트 · CI" 섹션 착수 — pytest 환경 구성(테스트 전용 DB) 완료 처리. 계획에 없던 작업 1건 추가 기록(스모크 테스트 4건 — 환경이 실제로 섰는지를 판정할 수단이 필요했고, 세 건만으로는 테스트 간 초기화가 검증되지 않아 "흔적을 남기는 테스트"를 앞에 배치). M1 기술 요소에 pytest-asyncio 추가. 실작업은 9/23 · 9/24, 커밋은 9/25. 테스트 DB 구성 방식(같은 컨테이너의 별도 DB · 마이그레이션으로 스키마 생성 · TRUNCATE 격리) 사유는 `DEVLOG` 2026-09-25 결정 기록 참고 |
