...
>
> 작성일: 2026-08-13 / 개정일: 2026-09-22 / 상태: v1.9
...
| F-28 | 만료 토큰 정리 | 만료 · 폐기된 `refresh_tokens` 행을 주기적으로 삭제. 현재는 로그인할 때마다 누적되기만 함 | 낮음 |
| F-29 | 모든 기기에서 로그아웃 | 해당 사용자의 살아 있는 refresh 토큰을 전부 폐기. 현재 로그아웃은 요청한 기기의 토큰만 폐기한다. 재사용 감지의 일괄 폐기와 같은 쿼리 구조라 구현 비용은 작다 | 낮음 |
| F-30 | 비슷한 게임 후보 제시 | 게임 등록 시 제목이 비슷한 기존 게임을 후보로 보여주고 사용자가 고른다 (PostgreSQL `pg_trgm` 유사도). 자동 병합은 하지 않는다 — 다른 게임이 합쳐지면 공유 테이블 `games`를 쓰는 모든 사용자의 기록이 엉뚱한 게임을 가리키게 된다 | 낮음 |
| F-31 | 게임 목록 검색 등록 | 제목을 직접 입력하는 대신 외부 게임 목록(Steam 등)에서 검색해 고르고, 고유 ID를 함께 저장한다. Steam에 없는 게임을 위해 직접 입력은 병행한다 (R-03) | 낮음 |
...
| R-04 | 게임 제목 표기 불일치 (한글/영문/부제) | 중복 등록 | `steam_appid`를 우선 식별자로 사용. 없으면 제목 정규화 후 비교. (`ERD` 2.4절) 뜻이 같은 다른 표기(`7`/`VII`, 한글/영문)는 정규화로 잡지 못하므로 F-23 · F-30으로 보완 |
...
- [x] 상태 관리 라이브러리 → **Pinia**
- [x] 통계 시각화 → **ECharts**
- [x] 장르 데이터 출처 (직접 입력 vs Steam 태그 활용) → **Steam 공식 장르 8개를 고정 목록으로 사용** (사용자 입력 불가. 목록과 선정 기준은 `ERD` 2.5절)
- [x] 비밀번호 해싱 라이브러리 → **argon2-cffi (Argon2id)**
- [ ] Redis 작업 큐 도구 (M9 착수 시)
- [ ] 배포 환경: 자가 서버(집) vs 클라우드 (M5 착수 시)
...
| v0.2 | 2026-08-13 | 3.1절 Steam 라이브러리 자동 동기화 (M4) 부분을 05.milestones.md 문서와 맞게 Steam 라이브러리 자동 동기화 (M5)로 변경 |
| v0.3 | 2026-08-14 | 10장 미결정 사항 중 "RN 환경" 결정 완료로 이관 (Expo vs CLI → CLI로 확정, M0 완료 시점) |
| v0.4 | 2026-08-17 | 10장 인증 방식 결정 완료로 이관 (토큰 vs JWT → DRF 기본 TokenAuthentication 확정), 4.3절에 비밀번호 정책 검증기 항목 추가 |
| v1.0 | 2026-09-18 | 스택 전환(RN + Django + MariaDB → Vue 3 + FastAPI + PostgreSQL)에 따른 전면 개정. 대상을 모바일 앱 → 웹으로 변경(3.2절 반전), 확장 기능 F-18~F-21 신설 및 F-13·F-14 승격, 3.3절을 4.4절로 통합, 플레이타임 출처 규칙(5.5절) 신설, R-02를 자동 동기화에 맞게 수정. 개정 전 문서는 `v0-rn-django` 태그, 전환 사유는 `DEVLOG` 결정 기록 참고 |
| v1.1 | 2026-09-18 | 5.5절 수동 등록 게임의 플레이타임 규칙 정정 (세션 합계로 대체 → 기준점 + 세션 증가분). 기존 규칙은 세션 기록 전 플레이타임이 사라지는 문제가 있었음 |
| v1.2 | 2026-09-18 | 4.0절 유즈케이스 다이어그램 추가 |
| v1.3 | 2026-09-18 | F-02에서 플랫폼 · 구매처 제외 (v0부터 데이터 모델에 대응 컬럼이 없었고, 이를 사용하는 통계도 없음). 4.4절 F-26으로 이관, 2장 게임 정의에서 플랫폼 표현 삭제 |
| v1.4 | 2026-09-20 | 10장 비밀번호 해싱 라이브러리 결정 완료로 이관 (passlib vs bcrypt vs argon2-cffi → argon2-cffi 확정). 4.4절에 F-27(이메일 인증) 추가 — 가입 시 계정 존재 여부 노출을 막으려면 메일 인프라가 필요해 v1.0 범위에서 제외. 사유는 `DEVLOG` 2026-09-20 결정 기록 참고 |
| v1.5 | 2026-09-20 | 4.4절에 F-28(만료 토큰 정리) 추가 — `refresh_tokens`는 폐기 기록을 남기기 위해 행을 지우지 않으므로(`ERD` 2.2절) 로그인할 때마다 누적된다. 정리 작업은 v1.0 범위 밖으로 둔다 |
| v1.6 | 2026-09-21 | 4.4절에 F-29(모든 기기에서 로그아웃) 추가 — 로그인은 기존 토큰을 폐기하지 않아 기기별 토큰이 공존하며, 로그아웃은 요청한 기기의 토큰만 폐기한다. 전체 폐기는 별도 기능이라 v1.0 범위 밖으로 둔다. 사유는 `DEVLOG` 2026-09-21 결정 기록 참고 |
| v1.7 | 2026-09-21 | 10장 장르 데이터 출처 결정 완료로 이관 (직접 입력 vs Steam 태그 → Steam 공식 장르 중 8개를 고정 목록으로). 자유 입력은 공유 테이블 `genres`를 오염시키고, Steam 태그는 분류가 너무 잘게 쪼개져 장르별 통계(F-11)가 의미를 잃음. 사유는 `DEVLOG` 2026-09-21 결정 기록 참고 |
| v1.8 | 2026-09-21 | 4.4절에 F-30(비슷한 게임 후보 제시) · F-31(게임 목록 검색 등록) 추가, R-04 대응에 정규화의 한계와 보완책 명시 — 제목 정규화는 같은 글자를 다르게 쓴 경우만 잡고, 뜻이 같은 다른 표기는 잡지 못함. 규칙이나 유사도로 자동 병합하면 다른 게임을 합치는 실수가 생겨 v1.0 범위에서는 하지 않음. 사유는 `DEVLOG` 2026-09-21 결정 기록 참고 |
| v1.9 | 2026-09-22 | 문서 참조 표기를 번호에서 문서 이름 기준으로 통일(예: "01~05 문서" → `REQUIREMENTS` · `ARCHITECTURE` · `ERD` · `UI_DESIGN` · `MILESTONES`. 내용 변경 없음) |
